...
contract TravelManager {
    ...
    function createShippingCompany(
        ...
        if (
            shipping_companies[shipper_wallet].shipper_wallet == shipper_wallet
        ) {
            return 0;
        }
    ...
    function getShippingCompany(address shipper_wallet)
        public
        view
        returns (ShippingCompany memory)
    {
        return shipping_companies[shipper_wallet];
    }

    function createDriver(string memory name, address payable driver_wallet)
        public
        returns (uint256)
    {
    ...
    function getDriver(address driver_wallet)
        public
        view
    ...
    function getClient(address client_wallet)
        public
        view
    ...
    function checkCurrentStatusUpdate(uint256 travel_id) private {
        ...
        if (
            _travel.status.shipping_company &&
            _travel.status.client &&
            _travel.status.driver
        ) {
            ...
            if (
                _travel.status.current_status == 3 &&
    ...
    function getTravels() public view returns (Travel[] memory) {
        ...
        for (uint256 i = 0; i < travels_size; i++) {
            ret[i] = travels[i];
        }
        return ret;
    }

    function alterAgreement(uint256 travel_id)
        public
        payable
        returns (uint256)
    {
        ...
        if (
            msg.sender == _travel.shipping_company.shipper_wallet &&
            _travel.status.shipping_company == false
        ) {
            if (_travel.status.current_status == 1) {
        ...
        if (
            msg.sender == _travel.driver.driver_wallet &&
            _travel.status.driver == false
        ) {
            if (_travel.status.current_status == 1) {
                payDriver1(_travel.driver.driver_wallet, travel_id);
        ...
        if (
            msg.sender == _travel.client.client_wallet &&
            _travel.status.client == false
        ) {
            if (
                msg.sender.balance > _travel.travel_cost &&
                _travel.status.current_status == 0
            ) {
